...
import { getTrendingTVSeries } from '@/lib/getTvSeries';
...
function TvSeriesList() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getTrendingTVSeries(activeType.value, currentPage.toString())
        .then(res => {
          setData(res.results);
        ...
        {data !== undefined && data?.length >= 1 ? (
            <div>
              <div className={`w-full h-auto max-w-[1800px] ${grid ? 'flex flex-col': 'grid 2xl:grid-cols-4 xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-3 md:gap-6 xl:gap-10'}`}>
                {data.map(((movie: TVShow) => 
                !grid ? <TvCard movie={movie} key={movie.id} /> : (
                  <div key={movie.id} className={"flex flex-col justify-start space-y-5 mb-5 items-center lg:flex-row space-x-5"}>
                    ...
                    <div className="max-w-4xl">
                      <Link href={`/tv/${movie.id + '-' + movie.original_name.replaceAll(' ', '-')}`}><p className="font-bold">{movie.original_name} {movie.first_air_date?.split("-")[0]}</p></Link>
                      <hr className="mb-3"/>
              ...
              <PaginationCustom currentPage={currentPage} totalPage={totalPage} setCurrentPage={setCurrentPage} />
            </div>
